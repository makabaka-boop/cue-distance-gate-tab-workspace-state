...
两个页签在页面中**始终同时挂载**，切换只切换显隐、不卸载组件。因此：

- 控制台的已打开场次（身份、版本、时间线）、场次名称、载入编号、cue 草稿、
  错误与忙碌态，以及偏差校验页的两个序列、K、在途请求与结论，往返页签后全部保留；
- 在途命令在原控制台内结算，响应只能更新**发出该命令时所属的场次**：
  若期间已载入另一场次，旧命令的快照不会切回视图、不会改动新场次版本/时间线，
  其成功后续（清空 cue 草稿）也只作用于原场次；
- cue 输入**不再在发送前清空**，而是在服务端确认提交后才清空——这样即使命令
  在途时切走、服务端实际已提交，返回后版本与时间线仍会更新；若被拒绝
  （如 `VERSION_CONFLICT`），草稿保留且可直接重试；
- 偏差校验的每次比较带单调请求身份（结论标注「第 N 次比较」）：重叠比较或
  页签往返后，较旧的响应（成功或失败）不会覆盖较新的结论；两个在途请求
  独立结算忙碌态；
- 一个入口的成功或失败都不会清除另一个入口的任何状态。

现有 API、全局 requestId 裁决、`expectedVersion` 版本冲突规则与偏差算法均不变。
...
# 前端页签保活/状态隔离验收（Vitest + Testing Library，可控 Promise 返回顺序：
# 请求前、提交后响应前、失败后往返切换，核对场次身份/版本/cue 唯一性/草稿/
# 忙碌态/错误/偏差结论归属）
cd web && npm test
```
